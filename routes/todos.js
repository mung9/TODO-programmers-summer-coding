...
router.put("/:id", async (req, res) => {
  ...
  let todo = _.pick(req.body, ["title", "content", "due", "done"]);
...
router.get("/:year/:month/:date?", async (req, res) => {
  ...
  if (!year || !month)
    return res.status(400).send("The given parameters are invalid.");
...
router.put("/:id", async (req, res) => {
  ...
  if (invalidIdErr)
    return res.status(400).send(invalidIdErr.details[0].message);

  let todo = _.pick(req.body, ["title", "date", "isCompleted", "updatedDate"]);
  const { error: invalidTodoErr } = validateTodo(todo);
  if (invalidTodoErr)
    return res.status(400).send(invalidTodoErr.details[0].message);
